...
import { lookupMultipleCnpjs } from "../services/cnpj-lookup.service.js";
...
export async function buscaEmailsRoutes(app: FastifyInstance) {
  app.get<{
  ...
  }>("/api/busca-emails", async (request, reply) => {
    ...
    const minResultados = request.query.minResultados
    ...
    if (comResultado.length === 0) {
      return {
        ...
        emails: [],
        message:
          "Nenhuma licitacao com resultado encontrada. Tente uma busca mais ampla.",
      };
    }
      ...
      const results = await Promise.allSettled(
        batch.map(async (lic) => {
          const fornecedores = await enrichLicitacao(
            ...
            true // skip slow fallback - just get data from BrasilAPI
          );
    ...
    if (cnpjsSemEmail.length > 0) {
      ...
      // Update fornecedores with newly found emails

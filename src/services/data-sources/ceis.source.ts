...
const API_URL = "https://api.portaldatransparencia.gov.br/api-de-dados/ceis";

export class CeisSource implements DataSource {
  readonly name = "ceis";
  readonly label = "CEIS (Empresas Inidôneas)";
  ...
  async fetch(config: DataSourceConfig): Promise<SourceResult[]> {
    ...
    const maxPages = Math.ceil((config.quantity || 100) / 15); // ~15 per page
    const cnpjMap = new Map<string, SourceResult>();

    logger.info(`CEIS: buscando empresas inidôneas (max ${maxPages} páginas)`);
    ...
    for (let page = 1; page <= maxPages; page++) {
      try {
        ...
        if (!res.ok) {
          throw new Error(`CEIS API: ${res.status} ${res.statusText}`);
        }

        const data = (await res.json()) as CeisRecord[];
        ...
        logger.error(`CEIS page ${page} error: ${err.message}`);
        break;
      }
    }
    ...
    if (cnpjs.length > 0) {
      logger.info(`CEIS: enriquecendo ${cnpjs.length} CNPJs`);
    ...
    logger.info(`CEIS: ${cnpjMap.size} leads de empresas sancionadas`);
